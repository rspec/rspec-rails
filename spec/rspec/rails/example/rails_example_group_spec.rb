    ...
    it 'will not leak enqueued ActiveJob jobs between examples', skip: !RSpec::Rails::FeatureCheck.has_active_job? do
      ...
      group =
        RSpec::Core::ExampleGroup.describe("A group", order: :defined) do
          ...
          # Assigning `ActiveJob::Base.queue_adapter` directly is not enough:
          # Rails resolves the adapter for a job class and for the test helpers
          # separately, and the two only agree when the adapter is handed over
          # through this hook.
